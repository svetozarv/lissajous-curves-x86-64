; Lissajous curve generator using AVX instructions

%macro V_COMPUTE_SINE 1
; %1 - ymm register containing the input values (x or y)
; uses ymm7, ymm8 as temporary registers
...
section .data
    ...
    step_8x:    dd 0.00200, 0.00200, 0.00200, 0.00200, 0.00200, 0.00200, 0.00200, 0.00200   ; step = start[-1] - start[0]
...
lissajousAVX:
    ...
    mov rbp, rsp
    ; --- prolog ---
    mov rcx, 500                            ; rcx <- iterations (t <= 1.0 - one rotation; 1 / step_8x = 500 iter) (DO NOT CHANGE)
    ...
    vmovaps ymm14, [step_8x]                ; ymm14 <- [ 0.002, 0.002, 0.002, 0.002 | 0.002, 0.002, 0.002, 0.002 ]
...
mainloop:
    ...
    V_COMPUTE_SINE ymm5                      ; ymm5 = [ sin(a*t3+delta) | sin(a*t2+delta) | sin(a*t1+delta) | sin(a*t0+delta) ]
    ...
    V_COMPUTE_SINE ymm6                      ; ymm6 = [ sin(b*t3) | sin(b*t2) | sin(b*t1) | sin(b*t0) ]
    ...
    ; ================== update t ==================
    vaddps ymm15, ymm15, ymm14               ; t += [step]
    vmovaps ymm5, ymm15                      ; ymm5 <- current t
    vmovaps ymm6, ymm15                      ; ymm6 <- current t
